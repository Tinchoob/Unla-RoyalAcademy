...
public class TurnoControlador {
	...
	public @ResponseBody List<Turno> alta(@RequestBody Turno[] turnoArr) {
		List<Turno> lstTurnoAgregado = new ArrayList<Turno>();
		
		for (Turno turno: turnoArr) {
			try {
				turno.setIdTurno(0);         //Para evitar que sobreescriba si se le manda algo con ID
				turnoABM.save(turno);
				lstTurnoAgregado.add(turno);
			}
			catch (Exception e){
				e.printStackTrace();
			}
